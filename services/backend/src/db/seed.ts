...
export async function seedDatabase() {
  ...
  try {
    ...
    for (const sender of trustedSenders) {
      await client.query(
        'INSERT INTO trusted_senders (id, domain, email, description, created_by) VALUES ($1, $2, $3, $4, $5)',
        [sender.id, sender.domain, sender.email, sender.description, sender.created_by]
      )
    }
