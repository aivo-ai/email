...
import { listMessagesSchema, getMessageSchema, sendEmailSchema } from '../schemas/backend.js';
export default async function mailRoutes(fastify) {
    ...
    fastify.get('/messages', {
        ...
        try {
            ...
            // Get total count
            const countResult = await db.query(`SELECT COUNT(*) FROM messages ${whereClause}`, params);
            const total = parseInt(countResult.rows[0].count);
            // Get messages
            const messagesResult = await db.query(`SELECT 
    ...
    fastify.get('/messages/:id', {
        ...
        try {
            ...
            const result = await db.query(`SELECT * FROM messages WHERE id = $1 AND user_id = $2`, [id, userId]);
            ...
            if (!message.is_read) {
                await db.query('UPDATE messages SET is_read = true, updated_at = CURRENT_TIMESTAMP WHERE id = $1', [id]);
            }
            reply.send({
                id: message.id,
