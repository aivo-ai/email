...
import { addTrustedSenderSchema } from '../schemas/backend.js';
export default async function policyRoutes(fastify) {
    ...
    fastify.get('/trusted-senders', {
        ...
        try {
            ...
            // Get total count
            const countResult = await db.query(`SELECT COUNT(*) FROM trusted_senders ${whereClause}`, params);
            const total = parseInt(countResult.rows[0].count);
            // Get trusted senders
            const result = await db.query(`SELECT id, email, domain, display_name, sender_type, created_at, updated_at 
    ...
    fastify.post('/trusted-senders', {
        ...
        try {
            ...
            const existingResult = await db.query('SELECT id FROM trusted_senders WHERE user_id = $1 AND (email = $2 OR domain = $3)', [userId, senderData.email || null, senderData.domain || null]);
            ...
            const result = await db.query(`INSERT INTO trusted_senders (user_id, email, domain, display_name, sender_type)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING *`, [
                userId,
    ...
    fastify.delete('/trusted-senders/:id', {
        preValidation: [fastify.authenticate],
        schema: {
            params: {
                type: 'object',
        ...
        try {
            ...
            const { id } = params;
            const result = await db.query('DELETE FROM trusted_senders WHERE id = $1 AND user_id = $2 RETURNING id', [id, userId]);
