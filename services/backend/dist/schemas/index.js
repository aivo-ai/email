...
const ceerionEmailSchema = z.string().email().regex(/^[a-zA-Z0-9._%+-]+@ceerion\.com$/, 'Email must be from ceerion.com domain');
// Auth schemas
...
}).refine(data => data.textBody || data.htmlBody, { message: 'Either textBody or htmlBody is required' });
...
}).refine(data => !data.email || data.email.endsWith(`@${data.domain}`), { message: 'Email must belong to the specified domain' });
