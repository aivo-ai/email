...
export function sanitizeEmailHtml(html, options = {}) {
    const { allowImages = false, mediaProxyUrl = 'http://localhost:8091/proxy', cidEndpointUrl = '/api/media/cid', messageId } = options;
    // Configure DOMPurify
    const config = {
    ...
    // Sanitize the HTML
    const cleanDom = purify.sanitize(html, config);
 ...
 */
export function generateEmailPreview(html, maxLength = 150) {
 ...
 */
export function detectUnsafeContent(html) {
...
export const stripHtmlTags = (html) => extractPlainText(html);
